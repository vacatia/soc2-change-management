...
1. Query Linear ISSUES for the `Major change` issue label first — this is the primary discovery query for designated Major changes. The label sits on the *SOC 2 Major Change* parent (checklist) issue, which IS the change record: one labelled parent issue = one Major change, whether that change is a whole project or one of several inside a longer-running project. It is how you find them efficiently; it does not by itself make the designation authoritative (see the criteria section). It is a grouped issue label: group "SOC 2 Change Management", child "Major change" (issue-label id `10f23bd2-beb1-4e66-a9e4-1bd0d64a6ef5`). For example:
   ...
   - Treat EVERY issue carrying `Major change` as a designated Major change unless a more recent decision comment says otherwise. Count each such labelled parent issue as one Major change.
   - **The checklist is the labelled issue's sub-issues.** The 8 required steps live as SUB-ISSUES (`children`) of the labelled *SOC 2 Major Change* parent issue, created from the *SOC 2 Major Change* issue template. Confirm the parent by its sub-issues being the 8 gates (R&D / Scoping, Impact Assessment, Authorization, Communication, Documentation, Pre-prod Testing, Rollback Plan, Post-Implementation Testing); its own title is usually "SOC 2 Major Change" but older ones vary (e.g. "SOC2 Compliance"), so match on the sub-issues, not the title. Health-check the SUB-ISSUES. A labelled issue with NO such sub-issues has no checklist — report that as the finding. Transitional allowance: the label may sit on a discrete change issue that is *associated with* rather than *being* the parent checklist issue — follow the association to the parent, health-check its sub-issues, and note the shape.
   ...
   - Also query the sibling issue labels `Minor change` (id `936ea0c0-6eb5-4d4a-9c4c-6b31136e6ad8`) and `Emergency change` (id `33423ab3-bed2-4130-b8d9-ee3f3e9038e9`), and treat those issues accordingly. An `Emergency change` parent issue's sub-issues are Approval + Post-mortem, not the 8 gates.
...
b. **Labels to reconcile** — the classification label is a non-authoritative reflection kept on the change's *record issue*, so surface two kinds of drift as one line each, and omit the sub-item entirely if nothing is out of sync:
   - *Stale vs. decision* — a record whose label contradicts its most recent decision comment: {ID + link} · "label says X, latest decision Y → update label to Y." The fix is always label→comment.
   - *Legacy project label (migration)* — a change still carrying a SOC 2 label on its **project** rather than on its parent/record issue: {project + link} · "move the {X} label to its *SOC 2 Major Change* parent issue." This clears as the label migration completes.
