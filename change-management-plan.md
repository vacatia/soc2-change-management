...
**Issue template — *SOC 2 Change Parent Issue*** (there are no SOC 2 project templates). One template creates the Parent Issue for any classification, replacing the former separate *SOC 2 Major Change* and *SOC 2 Emergency Change* templates. It seeds the title with the `Major:`/`Minor:`/`Emergency:` prefix placeholder and the Parent Issue ready to carry the classification label and decision comment. Its child issues are the classification's steps: the 8-step checklist (§4) for a Major, Approval + Post-mortem for an Emergency, none for a Minor. Because a Linear template's sub-issue set is fixed, the template ships **without children** (the Minor/default shape); the applier adds the classification's child set — the 8 gates for a Major, Approval + Post-mortem for an Emergency — from the matching child-issue template. (An acceptable alternative, if adding children by hand is unwanted, is thin per-classification variants that each produce a SOC 2 Change Parent Issue with its children pre-seeded.) For routine emergencies, approval is pre-granted under standing authorization — the first responder fixes immediately and writes the post-mortem, without waiting for sign-off.

A template can be applied at any time, so a change reclassified Major after it started follows exactly the same steps as one that starts out Major — apply the template, apply the label, add the child set — with no API scripting and no second path to document.
