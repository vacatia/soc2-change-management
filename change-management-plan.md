...
Classification is driven by the risk criteria above. Many changes are clearly minor or clearly major; where a change falls in a grey area, the review group of engineering and security leaders applies and documents its judgment, recording the decision and its rationale as a decision comment on the change's record issue (see §7). Because a SOC 2 Type 2 audit requires evidence that the control operated in practice, the team proactively ensures in-scope major changes are processed during the observation window — including deliberate practice runs across teams.

## 3. Linear setup
**Label group `SOC 2 Change Management`**, applied to **issues** — the change's record issue — never to projects:
- `Minor change`, `Major change`, `Emergency change`, one per record issue. The label is a non-authoritative *reflection* of the group's decision; the authority is the decision comment on that same issue (§7). `Major change` goes on the *SOC 2 Major Change* parent issue, `Emergency change` on the *SOC 2 Emergency Change* parent issue, `Minor change` on the change's own issue (no label also means Minor). Because the label sits on the record issue in every case, there is one consistent place to find a change's classification and its decision — and a long-running project that contains several discrete majors simply has several labelled parent issues, with no project-level designation required.
- **Project-level classification labels are retired.** They are being migrated onto record issues and archived (see appendix). Until migration completes, the scan still reads them and surfaces any not-yet-moved label as a reconciliation item, so nothing is lost in the interim.

**Issue templates** (there are no SOC 2 project templates):
- *SOC 2 Major Change* — one parent issue whose sub-issues are the 8 checklist steps (§4); carries the `Major change` label. Applied inside the relevant project.
- *SOC 2 Emergency Change* — one parent issue with Approval + Post-mortem sub-issues; carries the `Emergency change` label. Used for every break-glass change, standalone or inside a project. For routine cases, approval is pre-granted under standing authorization — the first responder fixes immediately and writes the post-mortem, without waiting for sign-off. (This replaces the former split between an Emergency *issue* template and an Emergency *project* template — the project template is retired, mirroring the Major change on 2026-08-03.)

An issue template can be applied at any time, so a change reclassified Major after it started follows exactly the same steps as one that starts out Major — apply the issue template, apply the label — with no API scripting and no second path to document.

## 4. Major-change checklist (8 steps)
The 8 steps are the sub-issues of one *SOC 2 Major Change* parent issue in the project — the same parent issue that carries the `Major change` label and the classification decision comment. They stay grouped under that parent rather than sitting loose among the team's development issues: the compliance record is legible on its own, the project board stays readable, and each step can still be assigned, estimated, and blocked like any other issue.
...
- **Major** → the owning team's engineering manager runs the process in Linear: inside the relevant project, add the *SOC 2 Major Change* issue template to create the parent issue and its 8 sub-issues, apply the `Major change` label to that parent issue, write the decision comment on it, complete the steps, and record who/when/why in the Authorization sub-issue. This is the same whether the change is a whole project or one of several discrete majors inside a longer-running project — each major is its own labelled parent issue. The next scan moves it to "in process" and health-checks it.
...
**Recording decisions:** after each review, the meeting notes (Gemini notes) with the group's calls are posted to #soc2-change-review — the visible, dated record the scan reads to know what's resolved. The owning team's engineering manager, or their responsible engineer under the EM's oversight, then applies the corresponding label to the change's **record issue** in Linear — never the project (§3). Note what that label is and is not: it marks the outcome, not the reasoning, it does not record who decided, and it is only a *reflection* of the decision — not the decision itself. The classification decision and its rationale live only in the meeting notes today — closing that gap is an open proposal to record them as decision comments in Linear. Those comments go on the **issue, never the project** — the *SOC 2 Major Change* parent (record) issue for a Major, the change's own issue otherwise — so there is one consistent place to find a decision across Minor, Major, and Emergency changes. The most recent comment on a record wins, so a reversal is just a newer comment. Because the label only reflects the decision, it can fall out of sync (e.g. after a Major → Minor reversal the `Major change` label is now stale), and during the migration a change may still carry its label on the project rather than its record issue; the scan reviews the change records and surfaces both — a label that no longer matches the latest decision, and a legacy project label to move onto the record issue — as updates for the owning team to make. It does not change the label itself. The Authorization sub-issue is a separate gate: it captures a business approver's validation that a change already classified Major is needed and correctly scoped, and it exists only for changes that reached that classification. The scan reports and keeps items visible until that Linear follow-through is done; it never makes Linear changes itself.
...
| Classification applied with documented judgment | Decision comment on the change's record issue (the labelled *SOC 2 Major Change* parent issue for a Major) recording the group's call and rationale; the issue-level classification label reflects it |
...
- Classification labels moved from projects to the change's **record issue** (the *SOC 2 Major Change* / *SOC 2 Emergency Change* parent issue, or a Minor's own issue), so label and decision comment sit together and discovery, saved views, and audit all key off issue labels. Project-level classification labels are retired: they are being migrated onto record issues and archived, and the scan surfaces any not-yet-moved project label as a reconciliation item during the transition. The Emergency *project* template is retired in the same move, leaving a single *SOC 2 Emergency Change* parent-issue template for every break-glass change.
